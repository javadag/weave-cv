import type { TCoreSection, TCoreSections, TPersonalContent } from "~/utils/schemas/content.schema"
...
export function resumeToText(personal: TPersonalContent | null, core: TCoreSections | null): string {
  ...
  const processEntry = (entry: TCoreSection["contents"][0], sectionType: string) => {
    if ("isHidden" in entry && entry.isHidden) return
    ...
    if (advancedTypes.has(sectionType)) {
      const adv = entry as {
        subtitle?: string
  ...
  for (const section of Object.values(core)) {
    ...
    for (const entry of section.contents) {
      processEntry(entry, section.type)
    }
    lines.push("")
  }
