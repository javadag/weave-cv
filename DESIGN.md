...
**Creative North Star: "The Honest Workspace"**

Weave CV's design system is built on the philosophy of an honest, well-organized workspace — a place where everything has its purpose, nothing is wasted, and the user feels immediately at home. The visual language is warm and inviting without being casual; professional without being cold. It respects the user's time and intelligence, offering clarity and confidence in equal measure.

The system embraces softness: rounded corners that invite touch, gentle shadows that suggest depth without drama, and a warm orange palette that evokes optimism and new beginnings. This is not a sterile corporate tool — it's a companion for one of life's most important tasks: landing your next role.

**Key Characteristics:**
- **Warm professionalism** — orange primary with amber accents, grounded by zinc neutrals
- **Soft tactility** — generous border-radius (12–16px), subtle shadows, inviting interactions
- **Spatial clarity** — generous whitespace, consistent rhythm, predictable hierarchy
- **Honest simplicity** — no dark patterns, no fake urgency, no decorative noise

## Colors

The palette is warm and optimistic, anchored by a vibrant orange that feels like sunrise — the promise of a new day, a new opportunity. Dark mode shifts to amber for eye comfort while maintaining the same warmth.
...
### Secondary
- **Amber Glow** (#f59e0b): The gradient partner. Used in the brand gradient (from orange-500 to amber-500) and as the dark-mode primary. Warmer and softer than the primary, it bridges the gap between bold CTAs and neutral backgrounds.
...
- **Dark Mode Surfaces** (#0b0b0e, #15151a, #1c1c21): Custom dark backgrounds that avoid the cold blue-black of many dark modes. These are warm, deep charcoals that let the orange accent breathe.
...
**Display Font:** Inter (with Vazirmatn for Persian/RTL, system-ui fallback)
**Body Font:** Inter (with Vazirmatn for Persian/RTL, system-ui fallback)

**Character:** Inter is the workhorse — clean, geometric, highly readable at all sizes. It's professional without being sterile, friendly without being whimsical. The tight tracking on headlines (-0.04em) gives display text a modern, confident feel. For RTL, Vazirmatn takes the lead — it's the Persian counterpart with matching optical weight and personality.
...
- **Label** (600, 0.75rem, 1.4): Small caps, section labels, metadata. Uppercase with wide tracking (0.05em) creates clear visual anchors.
...
**The RTL Swap Rule.** When `lang="fa"`, the font stack swaps to Vazirmatn first, Inter second. All spacing and layout uses logical properties (margin-inline-start, not margin-left) so the entire interface mirrors correctly.

## Layout

**Container:** max-width 1600px (the `--container-compact` token), centered with auto margins. This is generous enough for wide editor views but constrains content on ultra-wide screens.
...
- `lg` (1024px): Full 2-column layouts, expanded spacing

## Elevation & Depth

**Philosophy:** Soft, ambient depth. Shadows are never dramatic — they suggest layering and lift without creating visual noise. The system uses a hybrid approach: tonal layering (lighter/darker surfaces) for structural depth, and subtle shadows for interactive elements.
...
**Border Radius Scale:**
- `sm` (8px): Inputs, chips, small buttons
- `md` (12px): Standard buttons, cards
- `lg` (16px): Feature cards, elevated surfaces
...
- **Shape:** Softly rounded (12px radius), generous padding (14px 28px)
- **Primary:** Orange gradient background (from-primary-500 to-primary-700), white text. On hover: slight lift (translateY -1px), brightness increase, warm orange glow shadow.
- **Secondary:** Transparent background, orange text, orange border. On hover: subtle orange tint background (bg-primary/5).
- **Ghost:** No border, no background. Used for navigation links and tertiary actions.
- **Dashboard CTA:** Zinc-900 background, white text. On hover: lighter zinc. This creates visual hierarchy — the dashboard "go" button is deliberately muted compared to the landing page CTA.
...
### Do:
- **Do** use the warm orange palette consistently — it's Weave CV's signature
